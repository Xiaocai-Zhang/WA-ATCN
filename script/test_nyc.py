...
from processing.NYC.processing import processing
...
class config:
    ...
    date_split = '2019-05-20'
    data_path = './data/NYC/Passenger_flow.csv'
    ParaPath = './para/parameter_nyc.csv'
    SavePath = './para/test_result_nyc.csv'
    seed = 14
...
def Test(x,round_no):
    ...
    SaveModlPath = './save/NYC/round_' + str(round_no) + '/'
    ...
    np.savetxt('./prediction/NYC/prediction.csv', predictions_test, delimiter=',')
    np.savetxt('./prediction/NYC/groundtruth.csv', y_test_glo, delimiter=',')
